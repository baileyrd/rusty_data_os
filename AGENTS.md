...
sequence and `git diff --check` from the repository root. The Markdown link validator includes existing tracked and non-ignored untracked Markdown; CI checkouts are clean, so the untracked scan affects only local runs.
