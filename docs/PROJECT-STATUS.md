...
The R31-authorized fourth-crate extension now represents that injected observation plan,
exactly-once action boundary, frozen lifecycle, independently typed sources, fail-closed partial
outcome, and reverse-order cleanup contract with synthetic tests. This is bounded deterministic
orchestration correctness evidence only. It adds no live capture or target validation and does not
