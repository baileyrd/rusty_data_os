...
A separately owner-authorized controlled follow-up removed repeated whole-prefix lifecycle validation while preserving shared lifecycle transitions and every correctness gate. With byte-identical RF1 inputs, replay medians changed from 0.119032 to 0.062490 ms at 100 events (1.90x), 5.584829 to 0.379115 ms at 1,000 (14.73x), and 542.289996 to 2.502494 ms at 10,000 (216.70x). The implementation retains linear collection searches for some mixed lifecycle records. These local/container timings remain bounded exploratory evidence only and lift no broader execution, durability, confirmation, or architecture gate.
