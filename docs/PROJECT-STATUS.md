...
**Status:** Phase 1 planning/readiness — R21 freezes the context for a later complete R20 mapper gate; live capture remains blocked and execution unauthorized
...
The authorized pure mapper and deterministic tests provide bounded implementation/correctness-validation evidence only for locally decidable mapping rules; the complete R20 correctness gate is not closed. Duplicate reference bytes are rejected by SOP1 validation, but future, self, and cross-stream membership require prior-event and stream context absent from the frozen mapper state. R21 now supplies that governance freeze; later implementation and its reviewed correctness gate remain required for full mapper closure. They do not integrate with append/reopen, materialize or execute a workload, or establish durability, recovery, performance, or capture evidence. The live-Linux-capture decision remains open and blocks a descriptive D1 harness.

## R21 reference-context decision

[R21](experiments/EXP-0001/R21-REFERENCE-VALIDATION-CONTEXT.md) freezes a catalog built only from complete validated semantic streams and a separate caller-owned accepted-prefix state. Its typed, stream-bound identity entries and deterministic precedence resolve the R20 reference-context governance blocker and prospectively authorize only a later pure extension of `exp1-raw-append-replay::mapping`. The complete R20 gate remains open until that implementation is reviewed and passes CI. No Rust/Cargo change, append integration, workload execution, capture, durability, recovery, benchmark, or performance evidence is part of R21. The independent live Linux capture blocker still prevents a descriptive D1 harness.
