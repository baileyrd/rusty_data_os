...
| R27-authorized v2 reference-context implementation | Exact R27 APIs/errors/precedence/bounds/paths/tests; existing record-format and workload-conformance path dependencies | Pending bounded correctness gate for zero-target bootstraps, same-stream/same-segment prior references, complete-scope missing classification, and transactional state | No evidence until merged exact-head review and CI; every R27 exclusion remains |
