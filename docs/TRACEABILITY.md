...
| EXP-0001 | RQ-003; HYP-0001 (partial future evidence only); [execution-readiness plan](experiments/EXP-0001/EXECUTION-READINESS-PLAN.md); R1–R28 authorities | Proposed; bounded correctness slices through the merged R27 contextual mapper and complete R20 reference-context gate are closed; R28 authorizes only one test-only D1 integration, while live Linux capture, BLK-015, D2/D3, apparatus, execution, benchmarks, production, and later slices remain gated |
...
| R28-authorized test-only D1 integration (pending external review) | Existing R26 literals and RF1 expectations; merged R27 context mapper; R19 `RawAppender` and physical replayer | Implements one deterministic four-record integration test with pre-append discontinuity rejection, caller-owned state immutability/advancement, contiguous D1 receipts, explicit replay limits, and byte/record/sequence/ordinal/order identity | Merge or external-review evidence; harness/capture/execution; synchronization, durability, or recovery; every other R28 exclusion |
