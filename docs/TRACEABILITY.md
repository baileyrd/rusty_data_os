...
| EXP-0001 | RQ-003; HYP-0001 (partial future evidence only); [execution-readiness plan](experiments/EXP-0001/EXECUTION-READINESS-PLAN.md); R1–R27 authorities | Proposed; bounded correctness slices including merged R26 conformance are closed; R27 authorizes the still-absent pure v2 reference-context mapper, while complete R20 closure, live Linux capture, BLK-015, D2/D3, apparatus, execution, benchmarks, production, and later slices remain gated |
...
| R27-authorized v2 reference-context implementation | Exact R27 APIs/errors/precedence/bounds/paths/tests; existing record-format and workload-conformance path dependencies | Implemented on the active closure branch: homogeneous v2 scope, immutable typed catalog, private caller-owned prefix, zero-target bootstraps, same-segment prior references, and transactional RF1 mapping | Bounded correctness only after merged exact-head review and CI; every R27 exclusion remains |
