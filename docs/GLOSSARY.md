...
- **incarnation** — a monotonically increasing generation for one record UUID; deletion and recreation start a new generation, so predecessor updates and edges are rejected.
- **indeterminate outcome** — a transaction whose commit-boundary I/O did not return conclusive success; resolve its stable request identity through validated history before retrying.
...
- **externally reviewed observation** — a non-sensitive value transcribed by an external review of an owner-supplied artifact that is not repository-retained; it guides follow-up but is not a verified target fact.
- **repository-retained evidence** — a safely sanitized source artifact and its capture/redaction provenance retained for reproducible review.
