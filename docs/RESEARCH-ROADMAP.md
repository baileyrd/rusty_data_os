...
### R33 implementation branch status

The single R33-authorized target-preflight implementation is present on this branch and remains pending exact-head external review and CI. It is deterministic implementation/correctness work only; it does not authorize or establish live invocation, target observations, workloads, tracefs, benchmarks, durability, baselines, or production expansion.
