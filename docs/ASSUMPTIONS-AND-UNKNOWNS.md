...
| UNK-013 | Resolved as documentation design by R21 for the pure R20 mapper: complete validated semantic streams build an immutable typed/stream-bound catalog, while caller-owned accepted-prefix state determines prior/current/future membership with frozen precedence and bounds. Later implementation and its complete correctness gate remain open. |
...
That pure mapper now has bounded implementation/correctness-validation evidence only for locally decidable rules. The frozen state has no prior-event membership or stream identity, so future, self, and cross-stream reference rejection cannot be established; duplicate bytes alone are rejected by the SOP1 validator. R21 now supplies that governance freeze; full R20 closure requires its later implementation and reviewed correctness gate. This does not resolve the direct Linux capture decision, authorize a descriptive harness, or supply execution, durability, recovery, or performance evidence.

### R21 disposition

R21 resolves the reference-context governance blocker and UNK-013 as documentation design. It prospectively authorizes only a later pure catalog/context extension in `exp1-raw-append-replay::mapping`; the full R20 mapper gate remains open until implementation, exact-head review, and CI. Reconstruction, capture, harness construction, execution, durability, recovery, performance, and every R21 exclusion remain open or unauthorized.
