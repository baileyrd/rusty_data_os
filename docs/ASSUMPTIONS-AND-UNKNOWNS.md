...
| UNK-022 | Narrowed by R29–R35: the semantic mapping, Linux/x86_64 ABIs, first-cell sources, non-live orchestration, and internal adapter are frozen and deterministically implemented through the R32 implementation closed by R33. R34 preserves v1 and freezes v2 ownership correction; R35 aligns v2 with the existing compile rejection, so unsupported targets produce no runtime artifact while `not_acquired` remains required for valid-request open failure. Compilation proves only the Rust target. Fedora-release/effective-host validation, live interface use, retained observations, records, calibration/overhead, capture, tracefs confirmation/attribution, execution, and evidence remain open. |
