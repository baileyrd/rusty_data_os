...
The pure mapper and its deterministic tests now provide bounded implementation/correctness-validation evidence only for the locally decidable mapping subset. The complete R20 gate remains open: duplicate reference bytes are locally rejected, but the frozen API/state cannot decide prior-event membership, future/self status, or stream locality. R21 now supplies the separately reviewed governance freeze; later implementation and its full correctness gate remain required. No append/reopen integration or execution was added, and the live Linux capture blocker and all stated exclusions remain unchanged.

## R21 reference-context readiness update

[R21](EXP-0001/R21-REFERENCE-VALIDATION-CONTEXT.md) defines the missing context as an immutable catalog built from complete validated semantic-stream bytes plus a mutable accepted-prefix state bound to one stream. It freezes typed identity/fact membership, bounds, collisions, precedence, and atomic state transition, resolving the governance blocker and authorizing only its later pure `exp1-raw-append-replay::mapping` implementation. The complete R20 gate remains open pending that implementation and exact-head CI. Live Linux capture still independently blocks the descriptive D1 harness; no execution or evidence follows.
