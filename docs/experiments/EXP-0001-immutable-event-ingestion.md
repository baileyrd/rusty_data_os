...
A useful result may be that some durability modes are competitive while others are not, or that batching changes the economics enough to justify a particular acknowledgement model. EXP-0000 completion is readiness documentation, not permission to build or run this experiment. The readiness plan and R1 requirements record resolve BLK-002 and BLK-013 without evidence or implementation authorization. The [R2 requirements and reference-vector plan](EXP-0001/R2-DETERMINISTIC-WORKLOAD-BYTES-IDENTITY-REFERENCES-DIGEST-REQUIREMENTS.md) constrains BLK-006 through BLK-009, which remain open pending concrete selections, rationale, and stable vectors. The [R3 lifecycle contract](EXP-0001/R3-IDENTITY-TIME-SEQUENCING-RETRY-LIFECYCLE.md) resolves BLK-004/005/011/012 and further constrains BLK-007. BLK-001/003 and the other deferrals remain open. The owner-approved [R4 target/platform evidence boundary](EXP-0001/R4-FEDORA-44-BOSGAME-M5-TARGET-AND-PLATFORM-DURABILITY-CONTRACT.md) closes R4 for conditional planning from the externally reviewed `data-os-r4-final.5fk5yE-sanitized-v2.tar.gz`, which is not repository-retained because issue #26 excludes committing the archive. A predecessor archive—not the accepted v2 artifact—exposed a sensitive NVMe identifier; no archive, digest, or sensitive identifier is committed. The accepted v2 artifact passed the residual-identifier audit except for the owner-accepted public NTP source hostname, but its non-retained provenance does not become repository-verified experimental evidence. The [R5 B0/B1 physical-profile and adapter contract](EXP-0001/R5-B0-B1-PHYSICAL-PROFILES-AND-ADAPTER-CONTRACTS.md) is incomplete: it fixes the initial B1 append/error/synchronization API, but BLK-001/003 still govern durable request/event binding, reservation recovery, post-boundary envelope/integrity finalization, recoverable commit, and their exact synchronization sequence. BLK-016/017 are narrowed rather than resolved and the B0/B1 portions of BLK-019 remain incomplete. R5 remains active and R6 is not authorized. BLK-015 remains open for dependent D2/D3 claims and execution because final placement, exact PLP/controller protection, and empirical survival remain unverified; no capture work, code, Cargo file, fixture, validator, adapter, workflow, benchmark, fault action, machine change, implementation, or execution is authorized.
