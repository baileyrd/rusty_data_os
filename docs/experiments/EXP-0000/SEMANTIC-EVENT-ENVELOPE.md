...
| Observation time (observation-side lifecycle metadata; not part of the original envelope) | States when a particular reader or materialization could observe the event; it is not system acceptance or durability time and cannot be added to the observed canonical event. | Conditional; zero or more observation records may exist, with at most one value per declared observer/context unless that context defines otherwise | REQ-004; REQ-013; ADR-0002 | Observable by the applicable reader/materializer, after canonical commit | Not applicable to the original envelope; each observation record is immutable once recorded | observation provenance; temporal interpretation | Whether observation records remain derived metadata or are appended as separate canonical events, observer identity, multiplicity rules, capture point, timestamp representation, and clock source |
...
| Source/actor provenance | Identifies the applicable origin and/or actor responsible for the accepted fact. It is distinct from request correlation and causation. | Conditional; one or more values when source/actor provenance is applicable, otherwise none | REQ-008; REQ-010; ADR-0002 | Command received or validation; finalized by fact/event construction when applicable | Yes | provenance; integrity/audit interpretation | Applicability rules, provenance vocabulary, number and nesting of actors/sources, identity format, authentication binding, and encoding |
...
| Integrity metadata | Semantically binds an event to the integrity checks declared for its envelope/payload so corruption or truncation can be detected within a documented capability. It does not promise an algorithm not yet selected. | Conditional; one or more values when the declared integrity mode uses per-event metadata, otherwise none | REQ-008; ADR-0002 | No later than persistence attempt when required by the declared integrity mode; exact finalization point depends on unresolved integrity coverage/framing | Yes when present | replay validation; integrity | Minimum required integrity policy; supported modes; checksum/hash/MAC algorithm; coverage, placement, keying, granularity, representation, and torn-record strategy |
...
3. **Time meanings remain separate.** Effective time may precede system acceptance time. System acceptance, durability, and observation time describe different lifecycle facts and are not interchangeable.
   Observation time is observation-side lifecycle metadata outside the immutable original envelope. Recording an observation cannot mutate the observed event; if the observation must become canonical history, it is appended as a separate event.
...
|---|---|---|
| Command received | Originating request identity and any applicable initial source/actor provenance are available. | Requested intent only; not a canonical event. |
...
| Fact/event constructed | Logical information/entity identity, event/fact type, schema identity/version, effective time, opaque payload, envelope semantic version, applicable provenance, causal references, and correction/retraction targets are known. Permanent event identity is known no later than this stage, though its exact assignment authority/timing remains unresolved. | Candidate accepted fact; not yet canonically committed. |
| Sequenced | Deterministic local sequence/replay position is assigned. | Ordered candidate; still not canonically committed. |
| Persistence attempted | The selected persistence path is attempted; when the declared integrity mode requires per-event integrity metadata, it is available no later than this stage, subject to unresolved coverage/framing. | Failure may abandon the candidate; earlier memory visibility remains provisional. |
...
| Observable by readers/materializers | Observation time may be recorded once for each declared observer/context as observation-side lifecycle metadata. | The committed event remains unchanged; materialization and its observation metadata remain derived. A canonical observation requires a separately appended event. |
...
- When should observation-side metadata be retained, what identifies an observer/context, and when should an observation instead be appended as a separate canonical event?
...
- What minimum integrity policy, supported integrity modes, and semantic coverage are required before selecting an algorithm or framing?
