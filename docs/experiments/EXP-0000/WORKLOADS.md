...
For the single-producer reference case, operation-ordinal order is the predeclared canonical sequence/replay order. In a concurrent case without a controlled global submission schedule, each producer's local ordinal order is fixed but the cross-producer interleaving and resulting ordinal-to-canonical-sequence mapping are observed outputs. Effective-time ordering never replaces canonical sequence. System acceptance time, durability time, and observation time arise only at their approved lifecycle points and must not be fabricated as payload or generator inputs. Observation time remains outside the immutable original envelope. Every run records exactly one temporal profile and its version.
...
9. canonical sequence is unique and strictly monotonic, with no duplicated, omitted, or invented operation; the observed ordinal-to-sequence mapping is recorded and replay reproduces that canonical order. For the single-producer reference, the mapping follows predeclared ordinal order. A concurrent run follows a predeclared global order only when it declares a controlled global submission schedule; otherwise cross-producer interleaving and the mapping are observed outputs, and different systems are not required to produce identical cross-producer canonical order.
...
| Stream verification | Pre-generated location if used, byte length, operation count, and digest method/value; otherwise deterministic regeneration procedure and expected test-vector results. Result metadata records the observed ordinal-to-canonical-sequence mapping for concurrent runs. |
