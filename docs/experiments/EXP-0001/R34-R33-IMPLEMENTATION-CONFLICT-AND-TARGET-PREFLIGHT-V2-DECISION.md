...
**Status:** Complete documentation/governance decision; PR #114 is not mergeable; exactly one later
synthetic-tested v2 correction or superseding implementation PR is prospectively authorized
...
PR #114 was re-reviewed at exact head `41d30f3484609b6b6ec03918bfd04b03ac56273a`; both
...
must not merge or be treated as correctness evidence. This decision does not close it, bless its
head, or authorize further commits to it.
...
The corrected artifact schema identifier is exactly
`EXP-0001-R34/target-preflight-artifact-v2`. All R33 field order, mappings, source/perf order,
...
V2 does **not** add a governance escape hatch. `TargetPreflightCallDispositionV2` remains exactly
R33's four values: `request_invalid`, `completed`, `serialization_failed`, and
...
the artifact. Construction rejects every cross-field contradiction, including:
...
One later PR may correct or supersede PR #114 by implementing only v2 in the existing
`exp1-descriptive-d1-harness` target-preflight module and re-export. It may retain v1 types only as
inert compatibility definitions; it must not emit a newly constructed v1 artifact or accept mixed
v1/v2 values. Cargo manifests, lockfiles, dependencies, toolchain, workflows, fixtures, and the
