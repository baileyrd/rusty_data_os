...
| R5 — B0/B1 physical profiles and adapter contracts (**next; documentation design authorized**) | What conditional lower bounds and OS operations preserve the declared cells? Inputs: R1–R4 and baseline contract. | Documentation resolving BLK-016/017 and B0/B1 portions of BLK-019, with placement/PLP/survival preconditions explicit. | SQLite/RocksDB details; implementation, execution, final placement, PLP/protection verification, and empirical survival. | Profiles reviewed for equivalence; **documentation only, no code or execution**. | Mapping and error-path table review. | An API cannot meet the intended D-mode or assumes an unverified physical fact. |
