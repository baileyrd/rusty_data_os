...
**Status:** Active staged plan; Slice A closed, bounded Slice B implementation prospectively authorized, execution unauthorized
**Scope:** bridge from the completed EXP-0000 framework to an executable EXP-0001
**Evidence classification:** planning plus reviewed Slice A correctness-validation status; no benchmark, persistence, durability, or performance evidence
...
| Implementation readiness | Slice A complete; Slice B prospective | R10 must merge before the bounded Slice B mechanism may be implemented; every later slice requires a separate authorization. |
| Correctness-validation readiness | Slice A passed only | Stable fixtures, oracle, recovery rules, fault mechanisms, and validation procedures must exist for every applicable measured mode. |
...
| BLK-016 | B0 implementation profile | Resolved as documentation design by [R5](R5-B0-B1-PHYSICAL-PROFILES-AND-ADAPTER-CONTRACTS.md); [R10](R10-SLICE-A-CLOSURE-AND-SLICE-B-AUTHORIZATION.md) prospectively authorizes bounded process-local D0 implementation, while observations/evidence remain gated | BLK-001, BLK-006–BLK-007 | UNK-020 | Exact lower-bound operations and accounting | Slice B/baseline validation |
...
| BLK-026 | Reproducible toolchain/build configuration | R9 is resolved for Slice A; R10 prospectively reuses it unchanged for bounded Slice B, while later/native/benchmark-series builds remain open | BLK-014, BLK-018 | UNK-020, UNK-022 | Rust 1.89.0, target, flags, lock/build identity and offline reproduction procedure | Slice A authorization; series separately gated |
| BLK-027 | Cargo/workspace and CI bootstrap authorization | Resolved by merged R9 for its exact workspace; R10 permits unchanged reuse for bounded Slice B and authorizes no expansion | BLK-020, BLK-026, section 6 | UNK-020, UNK-022 | Reviewed layout, empty dependency allowlist, exact CI plan, and approval record | First Slice A code creation after R9 merge |
...
R9 authorized the now-merged Slice A implementation. R10 records that Slice A passed its continuation gate as correctness-validation evidence only and prospectively authorizes **Slice B — B0 in-memory lower bound** after R10 merges. Execution, benchmark, persistence, fault, and durability work remains unauthorized.
...
BLK-001 and applicable BLK-003 freezes required by physical-record work. R9 satisfied this gate for Slice A. R10 separately satisfies it prospectively for only the bounded Slice B mechanism without authorizing execution or all slices.
...
enters **Phase 1 planning/readiness**, not experimental execution. Slice A now provides bounded implementation/correctness-validation evidence only. R10 prospectively authorizes Slice B implementation after merge; EXP-0001 remains non-executable.
