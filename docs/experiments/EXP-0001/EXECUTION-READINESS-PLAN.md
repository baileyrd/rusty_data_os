...
| BLK-026 | Reproducible toolchain/build configuration | Resolved by R9 for Slice A; later-slice/native/benchmark-series builds remain open | BLK-014, BLK-018 | UNK-020, UNK-022 | Rust 1.89.0, target, flags, lock/build identity and offline reproduction procedure | Slice A authorization; series separately gated |
| BLK-027 | Cargo/workspace and CI bootstrap authorization | Gate satisfied prospectively by R9 for exactly Slice A; resolved when R9 is reviewed and merged | BLK-020, BLK-026, section 6 | UNK-020, UNK-022 | Reviewed layout, empty dependency allowlist, exact CI plan, and approval record | First Slice A code creation after R9 merge |
...
R8 completion preceded R9. R9 has now passed section 6 prospectively for **Slice A — deterministic physical-record fixtures and validators**; authorization becomes effective only when R9 is reviewed and merged. It authorizes no later slice, execution, benchmark, fault action, or durability claim.
...
BLK-001 and applicable BLK-003 freezes required by physical-record work. R9 is the separate planning increment that satisfies this gate for Slice A only; authorization is prospective until its merge and never authorizes all slices.
...
enters **Phase 1 planning/readiness**, not experimental execution. There is still no Phase 1 evidence. The corrective A1 candidate becomes implemented only after its exact head is reviewed and merged; Slice B remains unauthorized. R9 has passed section 6 prospectively for Slice A only; EXP-0001 remains non-executable.
