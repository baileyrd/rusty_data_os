...
R8 completion precedes R9 and every implementation slice. R9 is now the next readiness increment, but it may authorize exactly one slice only after section 6 and all slice-specific blockers pass; R8 completion alone authorizes no Cargo, fixtures, code, workflow, execution, or benchmark. This preserves independent review without bypassing the explicit implementation gate.
