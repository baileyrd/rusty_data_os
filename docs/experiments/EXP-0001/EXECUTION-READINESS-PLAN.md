...
The pure mapper and its deterministic tests implement only the locally decidable subset of that gate. Duplicate reference bytes reject through SOP1 validation, but future, self, and cross-stream checks require prior-event membership and stream-locality context that R20 did not freeze. R21 now supplies the separately reviewed governance freeze for that context; the complete R20 gate remains open pending implementation and review. This is bounded implementation/correctness-validation evidence only; it does not alter the still-open live Linux capture decision or authorize harness construction or execution.

## R21 reference-context gate update

[R21](R21-REFERENCE-VALIDATION-CONTEXT.md) selects an immutable typed identity catalog constructed from complete conformance-validated semantic streams and a separate mutable accepted-prefix state for one selected stream. It fixes bounds, collision rules, failure precedence, transactional state advance, and the exact later pure mapper surface. Only a bounded extension inside `exp1-raw-append-replay::mapping` and its deterministic tests is prospectively authorized; no manifest/dependency, authority-crate, append/reopen, or execution change is allowed. Full R20 closure still awaits the later implementation, exact-head review, and CI. Live Linux capture remains an independent blocker and no descriptive D1 harness is authorized.
