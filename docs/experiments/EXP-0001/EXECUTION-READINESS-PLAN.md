...
| BLK-020 | Benchmark harness architecture | Constrained by the R7 component/provenance graph; executable architecture remains open | BLK-009–BLK-010, BLK-019 | UNK-020, UNK-022 | Experimental component and dependency-boundary design | Code authorization |
...
| BLK-023 | Numeric threshold registry and rationale | Open; R8 records the exact owner decision and leaves every affected cell descriptive rather than inventing values | Primary-cell design, baseline profiles | UNK-008 | Reviewed versioned per-cell values with evidence/rationale | Confirmatory execution only |
...
| R8 — primary matrix, thresholds and statistical plan (**incomplete: owner threshold decision**) | What observations can decide predeclared claims? Inputs: interpretation contract, frozen profiles/environment. | [R8 authority](R8-PRIMARY-MATRIX-THRESHOLDS-AND-STATISTICAL-PLAN.md): exact 40-cell candidate matrix, blocked threshold skeleton, decision table, and BLK-024 statistical freeze. | Exploratory expansions. | Confirmatory-design approval; **no code by itself**. | Completeness and independent statistical review. | Unsupported rationale, excessive uncertainty, or material design change. |
