...
A separately owner-authorized [incremental lifecycle-validation comparison](../experiments/exploratory-history-views/INCREMENTAL-LIFECYCLE-VALIDATION-COMPARISON.md) retained byte-identical RF1 inputs and all correctness gates while changing the 10,000-event replay median from 542.289996 ms to 2.502494 ms (216.70x before/after); the 100- and 1,000-event ratios were 1.90x and 14.73x. This isolates the elapsed effect associated with removing repeated prefix validation, but does not attribute CPU time, prove linear complexity for mixed lifecycles, establish durable behavior, or prove HYP-0001.
